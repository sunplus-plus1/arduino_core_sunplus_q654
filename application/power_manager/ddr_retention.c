...
uint32_t ret_csrs[RET_CSRS_CNT];
uint32_t ret_csrs_restore[RET_CSRS_CNT];

struct dwc_phy_regs {
	uint16_t dwc_phy_reg[4096];	/* change the size here, (area >> 2) */
};
...
void ctl_apb_wr(uint32_t adr, uint32_t dat) 
{
	UMCTL2_REG(adr) = dat;
}

int ctl_apb_rd(uint32_t adr) 
{
	uint32_t value;
...
void dwc_ddrphy_apb_wr(uint32_t adr, uint32_t dat) 
{
	DWC_PHY_REG(adr)=dat;
}

int dwc_ddrphy_apb_rd(uint32_t adr) 
{
	uint16_t value;
...
void apb_ctl_before_retention(void)
{
	...
	MOON0_REG->sft_cfg[2]  = 0x00040004;  //RF_MASK_V_CLR(1 << 2);
	MOON0_REG->sft_cfg[2]  = 0x00080008;  //RF_MASK_V_CLR(1 << 3);
	MOON0_REG->sft_cfg[2]  = 0x00100010;  //RF_MASK_V_CLR(1 << 4);
	MOON0_REG->sft_cfg[2]  = 0x00200020;  //RF_MASK_V_CLR(1 << 5);
...
{
	#include <ddr_ret_save.h>

	uint32_t cm4_sram_base_addr = CM4_SRAM_RET_ADDRESS;  //S-RAM(CM4) CPU view	
	volatile uint32_t *ddr_ret = (uint32_t *)cm4_sram_base_addr;
...
void apb_ctl_after_retention(void)
{
	...
	wait_loop(1000);
	wait_loop(1000);
	MOON0_REG->sft_cfg[14] = 0x00010001;  // 1->0: assert DDRCTL APB RST_N
	MOON0_REG->sft_cfg[14] = 0x00080008;  // 1->0: assert DDRPHY APB RST_N
