...
#if 0
SPI_HandleTypeDef *hspi_dma;
void spi_dma_irq(void)
{
	HAL_SPI_DMA_IRQHandler(hspi_dma);
}
void TFT_eSPI::pushPixelsDMA(uint16_t* image, uint32_t len)
{
  ...
  if(init_flag == 0)
  {
	//  hspi_dma = HAL_SPI_Get_Handler();
	  HAL_NVIC_SetVector(SPI_MASTER3_IRQn, (uint32_t)spi_dma_irq);
	  init_flag = 1;
  }
  ...
  HAL_NVIC_EnableIRQ(SPI_MASTER3_IRQn);
  HAL_SPI_Transmit_DMA(hspi_dma, (uint8_t*)image, len << 1);
  while (hspi_dma->State == HAL_SPI_STATE_BUSY_TX); // wati dma done, otherwise pio will in irq.
  HAL_NVIC_DisableIRQ(SPI_MASTER3_IRQn); // disable irq for pio mode
}
#endif
