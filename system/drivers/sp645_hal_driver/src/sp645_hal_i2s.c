...
{
	MOON4_REG->sft_cfg[24] = 0xFFFF5333; // PLLA Disable 1.24[11]
	if(freq == PLLA_147M) {
		 ...
		 * M_SDM=0x3fd (1021)
		 */
		MOON4_REG->sft_cfg[25] = 0xFFFF0a11;	//[14:9] DIVN
		MOON4_REG->sft_cfg[26] = 0xFFFF0018;	//[5:4]  PH_STEP_SEL
							//[3:0]  PH_SEL
		MOON4_REG->sft_cfg[27] = 0xFFFF03eb;	//[10:0] K_SDM
		MOON4_REG->sft_cfg[28] = 0xFFFF03fd;	//[10:0] M_SDM
...
static void _i2s_pin_mux(I2S_HandleTypeDef *handle)
{
	...
	switch(handle->Index) {
		case INDEX_I2S0:
			...
			HAL_PINMUX_Cfg(PINMUX_AUD_AU_ADC_DATA0, 1);// [9] I2S0 data pin TX/RX mode on {x1}
			break;
		case INDEX_I2S1:
			HAL_PINMUX_Cfg(PINMUX_AUD_AU1_CK, 1);// [11] I2S1 bck/lrck pin on {x1}
			if(handle->FlagTxRx == I2S_TX)
				HAL_PINMUX_Cfg(PINMUX_AUD_AU1_DATA0, 1);// [9] I2S1 data pin select TX mode {x1}
			else
				HAL_PINMUX_Cfg(PINMUX_AUD_ADC1_DATA0, 1);// [14] I2S1 data pin select RX mode {x1}
			break;
		case INDEX_I2S2:
			HAL_PINMUX_Cfg(PINMUX_AUD_AU2_CK, 1);// [10] I2S2 bck/lrck on {x1}
			if(handle->FlagTxRx == I2S_TX)
				HAL_PINMUX_Cfg(PINMUX_AUD_AU2_DATA0, 1);// [8] I2S2 data pin select TX mode {x1}
			else
				HAL_PINMUX_Cfg(PINMUX_AUD_ADC2_DATA0, 1);// [13] I2S2 data pin select RX mode {x1}
			break;
	}
