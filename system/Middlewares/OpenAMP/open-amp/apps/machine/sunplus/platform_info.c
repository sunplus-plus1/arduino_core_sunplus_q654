...
/* processor operations for hil_proc for rproc. It defines
 * notification operation and remote processor management. */
extern struct remoteproc_ops sp_proc_ops;
...
{
	extern int metal_sp_irq_init(void);
	metal_sp_irq_init();
	return 0;
};
...
platform_create_proc(int proc_index, int rsc_index)
{
	...
	if (!remoteproc_init(&rproc_inst, &sp_proc_ops, &rproc_priv))
		return NULL;
			   ...
			   rpmsg_ns_bind_cb ns_bind_cb)
{
	...
	xil_printf("creating remoteproc virtio: %p\n", SHARED_MEM_PA);
	...
	xil_printf("initializing rpmsg vdev in mode %u\n", role);
	if (role == VIRTIO_DEV_MASTER) {
		void *shbuf;

		shbuf = metal_io_phys_to_virt(shbuf_io,
					      SHARED_MEM_PA + SHARED_BUF_OFFSET); // FIXME

		/* Only RPMsg virtio master needs to initialize the
		 * shared buffers pool
		 * Slave can set shared buffers pool to NULL
		 */
		pshpool = &shpool;
		rpmsg_virtio_init_shm_pool(pshpool, shbuf,
					   (SHARED_MEM_SIZE -
					    SHARED_BUF_OFFSET));
	}
	...
	xil_printf("initializing rpmsg vdev done\r\n");
