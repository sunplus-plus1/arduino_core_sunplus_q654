...
/* processor operations for hil_proc for CM4. It defines
 * notification operation and remote processor management. */
extern struct remoteproc_ops cm4_proc_ops;
...
{
	extern int metal_cm4_irq_init(void);
	metal_cm4_irq_init();
	return 0;
};
...
platform_create_proc(int proc_index, int rsc_index)
{
	...
	if (!remoteproc_init(&rproc_inst, &cm4_proc_ops, &rproc_priv))
		return NULL;
